...
export default function App() {
  ...
  const finalizeService = async (drafts) => {
    ...
    if (!service) return;
    const syncResult = await queue.enqueueService(service, drafts);
          ...
          <section className="panel compact complete">
            ...
            {queue.pendingCount > 0 ? (
              <div className="alert info">
                <Cloud size={18} />
                <span>
                  <strong>{queue.pendingCount} foto(s) continuam salvas no aparelho.</strong>
                  <small>{lastService?.error || queue.lastSyncError || 'O sistema tentará enviar novamente automaticamente.'}</small>
                </span>
              </div>
            ) : (
              <div className="alert success">Todas as fotos foram confirmadas no Drive e registradas na planilha.</div>
            )}
