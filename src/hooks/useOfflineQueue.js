...
export function useOfflineQueue(session, onUnauthorized) {
  ...
  const syncPending = useCallback(async () => {
    if (syncingRef.current || !session?.token) {
      return { synced: 0, failed: 0, error: '' };
    }

    syncingRef.current = true;
    setSyncing(true);
    const summary = { synced: 0, failed: 0, error: '' };
    ...
    try {
      ...
      for (const item of items) {
        ...
        if (response.unauthorized) {
          summary.failed += 1;
          summary.error = response.error || 'Sessão expirada.';
          onUnauthorized?.();
          break;
        }
        ...
        if (!response.success) {
          summary.failed += 1;
          if (!summary.error) summary.error = response.error || 'Falha ao enviar a foto.';
          await putHistory({
            ...item.history,
            status: 'PENDENTE',
      ...
      summary.failed += 1;
      summary.error = error?.message || 'Falha inesperada durante a sincronização.';
    } finally {
      setLastSyncError(summary.error);
