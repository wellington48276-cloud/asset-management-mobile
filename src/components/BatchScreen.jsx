      ...
      <div className="batch-form-card">
        {mode === 'range' ? (
          <div className="range-fields">
            <label className="field"><span>Chapa inicial</span><input inputMode="numeric" value={initialPlate} onChange={(event) => setInitialPlate(onlyDigits(event.target.value))} placeholder="288000" /></label>
            <div className="range-separator"><Route size={18} /></div>
            <label className="field"><span>Chapa final</span><input inputMode="numeric" value={finalPlate} onChange={(event) => setFinalPlate(onlyDigits(event.target.value))} placeholder="299000" /></label>
          </div>
        ) : (
          <label className="field single-field"><span>Chapa do bem</span><input inputMode="numeric" value={singlePlate} onChange={(event) => setSinglePlate(onlyDigits(event.target.value))} placeholder="288000" /></label>
        )}
      </div>
