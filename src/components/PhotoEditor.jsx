...
export default function PhotoEditor({ image, onCancel, onConfirm }) {
  ...
  const previewStyle = useMemo(() => {
    const width = quarterTurn ? displayBox.height : displayBox.width;
    const height = quarterTurn ? displayBox.width : displayBox.height;

    return {
      left: displayBox.x + (displayBox.width - width) / 2,
      top: displayBox.y + (displayBox.height - height) / 2,
      width,
      height,
      transform: `rotate(${normalizedRotation}deg)`
    };
  }, [displayBox, normalizedRotation, quarterTurn]);
